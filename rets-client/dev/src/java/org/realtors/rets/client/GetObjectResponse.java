...
public class GetObjectResponse implements Closeable {
	...
	public GetObjectResponse(Map headers, InputStream in,
			Closeable resources) throws RetsException {
		...
		boolean containsContentId = headers
				.containsKey(SingleObjectResponse.CONTENT_ID);
		// non multipart request that returns text/xml and does NOT have a
		// Context-ID header, must only be a non-zero response code
		boolean nonMultiPart_xml_withoutContentId = !this.isMultipart && isXml
				&& !containsContentId;
		// multipart request that returns text/xml can only be a non-zero
		// response code
		...
		if (multiPart_xml || nonMultiPart_xml_withoutContentId) {
			...
			try {
				...
				if (root.getName().equals("RETS")) {
					replyCode = NumberUtils.toInt(root
							.getAttributeValue("ReplyCode"));
				...
				throw new RetsException(
						"Malformed response [multipart="
								+ this.isMultipart
	...
	public String getBoundary() {
		...
		HeaderElement[] contentType = BasicHeaderValueParser.parseElements(
				contentTypeValue, new BasicHeaderValueParser());
		if (contentType.length != 1)
			throw new IllegalArgumentException(
					"Multipart response appears to have a bad Content-Type: header value: "
							+ contentTypeValue);

		NameValuePair boundaryNV = contentType[0]
	...
	private static String unescapeBoundary(String boundaryValue) {
		...
		if (boundaryValue.endsWith("\""))
			boundaryValue = boundaryValue.substring(0,
	 ...
	 */
	public <T extends SingleObjectResponse> GetObjectIterator<T> iterator()
			throws RetsException {
	 ...
	 * 
	 * @param bufferSize
	 *            How large a buffer should be used for underlying streams.
	 * 
	 * @throws RetsException
	 */
	public <T extends SingleObjectResponse> GetObjectIterator<T> iterator(
			int bufferSize) throws RetsException {
		if (this.exhausted)
			throw new RetsException(
					"response was exhausted - cannot request iterator a second time");
		...
		if (this.isMultipart) {
			try {
				return GetObjectResponseIterator.createIterator(this,
						bufferSize);
			} catch (Exception e) {
				throw new RetsException(
						"Error creating multipart GetObjectIterator", e);
			}
		}
		// no other possiblities
		return new NonMultipartGetObjectResponseIterator(this.headers,
				this.inputStream);
	}
