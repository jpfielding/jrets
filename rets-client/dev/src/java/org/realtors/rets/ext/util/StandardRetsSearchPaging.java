...
import org.realtors.rets.ext.SearchResultSetWrapper;
import org.realtors.rets.ext.db.SearchResultSetTable;
...
public class StandardRetsSearchPaging extends AbstractIterator<Table> {
	...
	public static Table compose(final StandardRetsSearchPaging tables) {
		return new CompositeTable(tables);
	}
	
}

class CompositeTable implements Table {
	
	private final Iterator<Table> tables;

	private Table current;

	public CompositeTable(Iterator<Table> tables) {
		this.tables = tables;
	}
	...
	public boolean hasNext() {
		...
		if (!this.tables.hasNext()) return false;
	...
	public List<String> next() {
		...
		if (!hasNext) throw new IllegalStateException("end of data reached");
		List<String> next = this.current.next();
